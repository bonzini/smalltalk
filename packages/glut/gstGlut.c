...
gst_glut_SendCallbackMessage(const char* msg, OOP* args, int nArgs) {
  ...
  indexOop = vm_proxy->strMsgSend(gst_glut_classPointer,
										  msg,
										  NULL) ;
  if(indexOop == nil) return ;
  ...
  receiver = vm_proxy->strMsgSend(callbackList, 
										  "at:", 
										  vm_proxy->intToOOP(1),
										  NULL) ;
  selector = vm_proxy->strMsgSend(callbackList, 
										  "at:", 
										  vm_proxy->intToOOP(2),
										  NULL) ;
  ...
  if(nArgs > 0)
	...
	vm_proxy->perform(receiver, selector) ;
